...
long long SwapnScore(BoardManager &solution, long long i, long long j,
                     ...
                     long long iter, long long &tabuFlag, long long L=10) {
    long long nCollisions = solution.getNCollisions();

    tabuFlag = swapPair(solution, i, j, tabu, iter);
    return solution.getNCollisions() - nCollisions;
}
...
long long tabuLocalSearch(BoardManager &initSolution, long long steps) {
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        getRandomPair(initSolution, a, b); 
        score = SwapnScore(initSolution, a, b, tabu, i, tabuFlag, bSize - freeIdx);
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        input_stream >> boardManager;
        input_stream.close();
    } else {
        std::cin >> boardManager;
    }
    if (argc > 2) {
    ...
    long long success;
    success = RandomMetaSearch(getRandomLineFreeInit, tabuLocalSearch,
                                         boardManager, solution, LLONG_MAX, LLONG_MAX);
    if (argc > 2) {
        solution.showBoard();
    }
